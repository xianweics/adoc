...
const createRequest = (localStorage, subUrlKey) => {
  ...
  const request = axios.create({
    baseURL: formatFullPath({ protocol, address, port, subUrl: service[subUrlKey] })
  ...
  request.interceptors.request.use(
    (config) => {
      ...
      if (config.url === 'setToken') {
        // 重定向到setToken接口
        config.baseURL = formatFullPath({ protocol, address, port, subUrl: 'serviceAuth' })
      ...
      return new Promise((resolve) => {
        try {
          ...
          if (code === 200) {
            ...
            // 访客令牌过期，重新获取访客令牌
            request.get('setToken').then(response => {
