...
import { formatFullPath, wrapperResponse } from '../../utils.js'
      ...
      jwt.verify(refreshToken, secretRefreshKey, function (err, result) {
        if (err) {
          if (err.name === 'TokenExpiredError') {
            // 刷新令牌过期
            ctx.body = wrapperResponse(responseCodeMap.REFRESH_TOKEN_EXPIRATION)
          } else {
            // 刷新令牌无效
          ...
          ctx.body = wrapperResponse({ ...responseCodeMap.SUCCESS, data: { accessToken: accessTokenRestore }, message: null })
          isPass = false
        }
      })
    } else {
